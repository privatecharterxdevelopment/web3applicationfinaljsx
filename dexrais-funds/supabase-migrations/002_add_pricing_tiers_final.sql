...
-- Create indexes
-- Simple index on featured_until (removed WHERE clause with NOW() which is not immutable)
...
SELECT
  ...
  CASE 
    WHEN c.featured_until IS NOT NULL AND c.featured_until > CURRENT_TIMESTAMP THEN true
    ELSE false
  END as is_featured
FROM campaigns c
...
-- Query to get currently featured campaigns (use in application code)
...
-- Query to get user's total revenue (platform owner)
-- SELECT SUM(launch_fee_paid_amount) as total_revenue
-- FROM campaigns
-- WHERE launch_fee_paid_amount IS NOT NULL;

-- Query to get revenue breakdown by tier
-- SELECT 
--   pricing_tier,
--   COUNT(*) as campaigns,
--   SUM(launch_fee_paid_amount) as revenue
-- FROM campaigns
-- WHERE launch_fee_paid_amount IS NOT NULL
-- GROUP BY pricing_tier
