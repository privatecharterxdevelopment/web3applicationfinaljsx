        ...
        <>
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-semibold text-gray-900">No AI Requests Yet</h3>
              <p className="text-gray-500 mt-2 max-w-md mx-auto">
                Your AI-assisted booking requests will appear here. Start a conversation with our AI assistant to get personalized quotes and bookings.
              </p>
            </div>
          ) : (
                ...
                <div key={request.id} className="bg-white/60 backdrop-blur-sm rounded-xl border border-gray-200 overflow-hidden hover:border-gray-300 transition-colors">
                  <div className="p-4">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            ...
                            <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-gray-200 text-gray-700 text-xs font-medium rounded-full">
                              <Sparkles size={10} />
                              AI Chat
                            </span>
                          </div>
                      ...
                      <div className="bg-gray-100 p-3 rounded-lg mb-4">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm font-medium text-gray-900">
                            {parseRoute(request.data?.route || request.data?.Route)}
                          </span>
                        </div>
                      </div>
                    )}
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
                      ...
                      {(request.data?.RequestedAt || request.data?.requestedAt || request.data?.date) && (
                        <div className="bg-gray-50 p-3 rounded-lg">
                          <div className="text-xs text-gray-500 mb-1">Requested</div>
                          <div className="text-sm font-medium text-gray-900">
                            {formatDate(request.data?.RequestedAt || request.data?.requestedAt || request.data?.date)}
                          </div>
                        </div>
                      )}
                      {(request.data?.Type || request.data?.type) && (
                        <div className="bg-gray-50 p-3 rounded-lg">
                          <div className="text-xs text-gray-500 mb-1">Type</div>
                          <div className="text-sm font-medium text-gray-900 capitalize">
                            {(request.data?.Type || request.data?.type || '').replace(/_/g, ' ')}
                          </div>
                        </div>
                      )}
                    </div>
                    ...
                    {(request.data?.conversationSummary || request.data?.ConversationSummary) && (
                      <div className="bg-gray-50 rounded-lg overflow-hidden mb-4">
                        ...
                        <div className="p-4 max-h-64 overflow-y-auto space-y-3">
                          {formatConversationSummary(request.data?.conversationSummary || request.data?.ConversationSummary).map((msg, idx) => (
                            <div key={idx} className={`flex gap-3 ${msg.role === 'user' ? '' : 'flex-row-reverse'}`}>
                              <div className={`flex-shrink-0 w-6 h-6 rounded-full flex items-center justify-center text-xs ${
                                ...
                                {msg.role === 'user' ? 'U' : msg.role === 'results' ? 'R' : 'S'}
                              </div>
                              <div className={`flex-1 p-3 rounded-lg text-sm ${
                                msg.role === 'user'
                                  ? 'bg-gray-900 text-white'
                    ...
                    {request.admin_notes && (
                      <div className="bg-gray-100 p-4 rounded-lg border border-gray-200">
                        <div className="text-xs font-medium text-gray-700 uppercase mb-2">Coordinator Response</div>
