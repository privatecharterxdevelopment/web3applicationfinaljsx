...
import { X, Building2, Mail, Phone, Globe, FileText, User, Upload } from 'lucide-react';
...
export const AddLeadModal: React.FC<AddLeadModalProps> = ({ 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Create the lead (using partners table)
      ...
      showSuccess('Success', 'Lead has been added successfully');
      onLeadAdded();
      onClose();
      resetForm();
    } catch (err: any) {
      console.error('Error adding lead:', err);
      showError('Error', err.message || 'Failed to add lead');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <h2 className="text-xl font-semibold text-black">Add New Lead</h2>
          <button
            onClick={onClose}
