...
import {
  Star, Wifi, Car, Coffee, Dumbbell, Waves, Utensils, ChevronLeft,
  Calendar, Bed, Check, Loader2, Users, MapPin, Building2, Heart, Share2
} from 'lucide-react';
...
const HotelDetailInline = ({ hotelData, onBack }) => {
  const navigate = useNavigate();
  const { user } = useAuth();

  // Get hotel from passed data
  const passedHotel = hotelData?.hotel;
  const passedRooms = hotelData?.availableRooms;

  // Hotel state
  const [hotel, setHotel] = useState(passedHotel || null);
  const [hotelReviews, setHotelReviews] = useState([]);
  const [isLoading, setIsLoading] = useState(!passedHotel);

  // Booking state
  const [activeTab, setActiveTab] = useState('details');
  const [checkInDate, setCheckInDate] = useState(hotelData?.checkIn || '');
  const [checkOutDate, setCheckOutDate] = useState(hotelData?.checkOut || '');
  const [adults, setAdults] = useState(hotelData?.adults || 2);
  const [rooms, setRooms] = useState(hotelData?.roomCount || 1);
  ...
  useEffect(() => {
    ...
    // If we have a hotel, try to fetch reviews
    fetchHotelReviews();

    // Set default dates if not passed
  ...
  const setDefaultDates = () => {
    ...
    if (!checkInDate) setCheckInDate(tomorrow.toISOString().split('T')[0]);
    if (!checkOutDate) setCheckOutDate(dayAfter.toISOString().split('T')[0]);
  };

  const fetchHotelReviews = async () => {
    if (!passedHotel?.hotelId) return;
    try {
      const { data: reviewsData } = await hotelService.getHotelReviews(passedHotel.hotelId);
  ...
  // Generate rooms if not provided
  const hotelRooms = passedRooms?.length > 0 ? passedRooms : hotel?.rooms || [
    {
      roomId: 'deluxe-1',
      ...
      rates: [
        { rateId: 'rate-1', rateName: 'Best Available', totalRate: hotel?.minRate || 250, currency: 'USD', boardType: 'Room Only', cancellation: { refundable: true } },
        { rateId: 'rate-2', rateName: 'With Breakfast', totalRate: (hotel?.minRate || 250) + 45, currency: 'USD', boardType: 'Breakfast', cancellation: { refundable: true } }
      ]
    },
    {
      roomId: 'suite-1',
      ...
      rates: [
        { rateId: 'rate-3', rateName: 'Suite Rate', totalRate: (hotel?.minRate || 250) * 1.8, currency: 'USD', boardType: 'Breakfast', cancellation: { refundable: true } }
      ]
    }
  ];
  ...
  return (
    <div className="w-full flex-1 flex flex-col" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {/* Hotel Info */}
          <div className="bg-white/35 rounded-2xl border border-gray-300/50 p-6" style={{ backdropFilter: 'blur(20px) saturate(180%)' }}>
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              {(hotel.amenities || []).map((amenity, idx) => (
                <span
                  key={idx}
                  className="flex items-center gap-1.5 px-3 py-1.5 bg-gray-100/50 rounded-lg text-xs text-gray-600"
                >
            ...
            {/* Check-in/out times */}
            <div className="mt-4 pt-4 border-t border-gray-200/50 flex gap-6">
              <div>
          ...
          {/* Tabs */}
          <div className="flex gap-1 bg-gray-100/60 p-1 rounded-xl" style={{ backdropFilter: 'blur(20px) saturate(180%)' }}>
          ...
          {/* Tab Content */}
          {activeTab === 'details' && (
            <div className="bg-white/35 rounded-2xl border border-gray-300/50 p-6 space-y-4" style={{ backdropFilter: 'blur(20px) saturate(180%)' }}>
              <h3 className="font-medium text-gray-900">About this hotel</h3>
              <p className="text-sm text-gray-600 leading-relaxed">
                {hotel.description || 'Experience luxury and comfort at this exceptional hotel, featuring world-class amenities and outstanding service.'}
              </p>
          ...
          {activeTab === 'rooms' && (
            <div className="space-y-4">
              {hotelRooms.map(room => (
                <div key={room.roomId} className="bg-white/35 rounded-2xl border border-gray-300/50 p-6" style={{ backdropFilter: 'blur(20px) saturate(180%)' }}>
                  ...
                  <div className="flex flex-wrap gap-2 mb-4">
                    {(room.amenities || []).slice(0, 6).map((amenity, idx) => (
                      <span key={idx} className="px-2 py-1 bg-gray-100/50 rounded text-xs text-gray-600 capitalize">
                        {amenity}
                      </span>
                    ))}
                  </div>
                    ...
                    {room.rates.map(rate => (
                      ...
                      >
                        ...
                        <div className="text-right">
                          <p className="font-semibold text-gray-900">${Math.round(rate.totalRate)}</p>
              ...
              {hotelReviews.length > 0 ? (
                <div className="space-y-4">
                  {hotelReviews.map((review, idx) => (
                    <div key={idx} className="pb-4 border-b border-gray-200/50 last:border-0">
        ...
        <div className="lg:col-span-1">
          <div className="sticky top-4 bg-white/35 rounded-2xl border border-gray-300/50 p-6 space-y-4" style={{ backdropFilter: 'blur(20px) saturate(180%)' }}>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-3 py-2 border border-gray-200 rounded-lg text-sm focus:ring-1 focus:ring-gray-900 focus:border-gray-900 bg-white/50"
                  />
                </div>
              </div>
              <div>
            ...
            <div>
              ...
              <div className="flex items-center border border-gray-200 rounded-lg bg-white/50">
                <button
                  onClick={() => setAdults(Math.max(1, adults - 1))}
                  className="px-3 py-2 hover:bg-gray-100/50 transition-colors"
                >
                  -
                </button>
            ...
            {selectedRate && (
              <div className="bg-gray-100/50 rounded-xl p-4 space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="text-gray-900">${Math.round(selectedRate.totalRate)}/night</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-500">{nights} night{nights > 1 ? 's' : ''} × {rooms} room{rooms > 1 ? 's' : ''}</span>
                  <span className="text-gray-900">${Math.round(totalPrice)}</span>
                </div>
            ...
            {selectedRate ? (
              <BuyWithCryptoButton
                ...
                hotelData={{
                  ...
                  guests: adults,
                  rooms: rooms,
                  pricePerNight: selectedRate.totalRate,
...
export default HotelDetailInline;
