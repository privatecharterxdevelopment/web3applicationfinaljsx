...
export const PVCX_TOKEN_ADDRESS = {
  ...
  mainnet: '0x1234567890123456789012345678901234567890' as `0x${string}`,
...
class Web3Service {
  ...
  /**
   * Fetch wallet transactions from Alchemy API
   * This will fetch all transactions (sends, receives, contract interactions)
   * for the given wallet address on the Base network
   */
  async getWalletTransactions(userAddress: `0x${string}`, options?: {
    limit?: number;
    fromBlock?: number;
    ...
    try {
      ...
      for (const transfer of allTransfers) {
        // Skip duplicates (same tx can appear in both sent and received if it has multiple operations)
        ...
        // Build token transfers array if applicable
        ...
        const transaction: WalletTransaction = {
          ...
          status: 'success', // Alchemy only returns confirmed transactions
          type,
          gasUsed: transfer.metadata?.gasUsed || undefined,
          gasPrice: transfer.metadata?.gasPrice || undefined,
          methodId: undefined, // Not provided by Alchemy transfers API
          tokenTransfers: tokenTransfers.length > 0 ? tokenTransfers : undefined,
          etherscanUrl: `${CHAIN.blockExplorers?.default?.url}/tx/${transfer.hash}`
        };
  ...
  /**
   * Get recent transactions (shorthand method)
   */
  ...
  /**
   * Request wallet signature for waitlist join
   * This creates a signed message that proves wallet ownership
   */
  async requestWaitlistSignature(
    walletClient: any,
    address: `0x${string}`,
    launchName: string,
    launchId: string
    ...
    try {
      ...
      const message = `I want to join the waitlist for "${launchName}".\n\nLaunch ID: ${launchId}\nWallet: ${address}\nTimestamp: ${timestamp}\n\nThis signature proves wallet ownership.`;
  ...
  /**
   * Verify a signature (optional - for security)
   */
...
// ===== AI CHAT REQUEST SIGNATURE =====
/**
 * Creates a signature message for AI Chat requests
 * Used when user selects blockchain services (CO2 certificate, crypto payment)
 */
export interface AIChatRequestSignature {
  requestId: string;
  userId: string;
  services: string[];
  totalAmount: number;
  ...
  includesCO2Certificate: boolean;
  includesCryptoPayment: boolean;
}

/**
 * Generate signature message for AI Chat request
 */
export function generateSignatureMessage(requestData: AIChatRequestSignature): string {
  const message = `
PrivateCharterX AI Chat Request Signature

Request ID: ${requestData.requestId}
User ID: ${requestData.userId}
Services: ${requestData.services.join(', ')}
Total Amount: ${requestData.totalAmount} ${requestData.currency}
Timestamp: ${new Date(requestData.timestamp).toISOString()}

Blockchain Features:
${requestData.includesCO2Certificate ? '✓ CO2 Certificate (Blockchain)' : ''}
${requestData.includesCryptoPayment ? '✓ Crypto Payment' : ''}

By signing this message, you authorize PrivateCharterX to process your request.
...
/**
 * Sign AI Chat request (to be called with wagmi signMessage)
 */
export async function signAIChatRequest(
  requestData: AIChatRequestSignature,
  signMessageFn: (args: { message: string }) => Promise<string>
): Promise<{ signature: string; message: string }> {
  const message = generateSignatureMessage(requestData);
  ...
  try {
    ...
    console.error('Failed to sign AI Chat request:', error);
    throw new Error('User rejected signature request');
  }
}
